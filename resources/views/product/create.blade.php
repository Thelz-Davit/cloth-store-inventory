                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label for="roundText">SKU <span class="text-danger">*</span></label>
                            <input type="text" id="roundText"
                                class="form-control round @error('sku') is-invalid @enderror" placeholder="SKU"
                                name="sku" value="{{ old('sku', $isEdit ? $product->sku : '') }}" />
                            @error('sku')
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" id="roundText"
                                ...
                                name="name" value="{{ old('name', $isEdit ? $product->name : '') }}" />
                            @error('name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select id="unit_code" name="unit_code"
                                ...
                                @foreach ($units as $unit)
                                    <option value="{{ $unit->code }}"
                                        {{ old('unit_code', $isEdit ? $product->unit_code : '') == $unit->code ? 'selected' : '' }}>
                                        {{ $unit->name }}</option>
                                @endforeach
                            </select>
                            @error('unit_code')
