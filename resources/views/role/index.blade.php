...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('role.delete') }}" method="POST" id="form-delete-selected" data-use-loader="true">
                @csrf

                <div class="d-flex mb-3">
                    <a href="{{ route('role.create') }}" class="btn round btn-primary">
                    ...
                    <table class="table" id="table1">
                        ...
                        <tbody>
                            @foreach ($roles as $index => $role)
                                <tr>
                                    <td>
                                        <input type="checkbox" class="row-check" value="{{ $role->role_id }}">
                                    </td>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $role->role_name }}</td>
                                    <td>
                                        <a href="{{ route('role.edit', $role->role_id) }}" class="btn btn-sm btn-primary">
        ...
        $(function() {
            ...
            btnDelete.on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    form.find('input[name="selected_roles[]"][data-hidden="1"]').remove();
